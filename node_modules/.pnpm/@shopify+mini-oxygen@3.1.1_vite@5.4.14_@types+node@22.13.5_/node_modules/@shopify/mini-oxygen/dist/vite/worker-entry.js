...
async function handleHMRPayload(runtime2, payload) {
  ...
  if (!(!hmrClient || runtime2.isDestroyed()))
    switch (payload.type) {
      ...
      case "full-reload": {
        const { triggeredBy } = payload, clearEntrypoints = triggeredBy ? [...runtime2.entrypoints].filter((entrypoint) => runtime2.moduleCache.isImported({
...
function fetchEntryModule(publicUrl, env) {
  ...
  return runtime.executeEntrypoint(env.__VITE_RUNTIME_EXECUTE_URL).catch((error) => {
    return {
